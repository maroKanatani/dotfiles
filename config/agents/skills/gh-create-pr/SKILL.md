...
5. コミットが必要な場合は、関係するファイルをパス指定でステージする。`git add .`と`git add -A`は使わない。コミットメッセージはConventional Commits形式で、作業の契機ではなく具体的な変更内容を書く。紐づくIssueがある場合は末尾にIssue番号を付与する。
6. push前に、現在のブランチと送信先remoteを確認する。
7. PRタイトルと本文を作る。本文はリポジトリのテンプレートを保ち、変更内容、判断理由、検証結果を記載する。自動生成署名は追加しない。
8. `gh pr create`でbaseとheadを明示する。ユーザーがReady状態を明示した場合だけReadyで作成し、それ以外は`--draft`を指定する。
...
```text
fix: PR作成時のDraft既定値を追加 #xxx
```

`fix: レビュー指摘に対応`のように差分を特定できないメッセージは使わない。

## 完了条件

- PRに依頼対象の変更だけが含まれている。
- PR本文がテンプレートに従い、検証結果と残るリスクを示している。
- ユーザーがReady状態を明示していないPRはDraftになっている。
- 指定されたメタデータが反映されている。
